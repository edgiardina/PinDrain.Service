...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Tab switching with better organization and debugging
  function showPane(activeTab, activePane) {
    console.log('Switching to tab:', activeTab.textContent);
    console.log('Target pane:', activePane.id);
    
    // Remove active from all tabs
    [tabQuad, tabRoi, tabLive, tabUrls, tabStatus].forEach(tab => {
      if (tab) {
    ...
    if (activeTab) {
      ...
      console.log(`Activated tab: ${activeTab.textContent}`);
    }
    if (activePane) {
      activePane.classList.add('active');
      console.log(`Shown pane: ${activePane.id}`);
    }
  ...
  // Tab event handlers with enhanced debugging
  if (tabQuad) {
    tabQuad.onclick = () => {
      console.log('Camera Setup tab clicked');
      showPane(tabQuad, paneQuad);
    };
  }
  if (tabRoi) {
    tabRoi.onclick = () => {
      console.log('ROI Config tab clicked');
      showPane(tabRoi, roiPane);
    };
  }
  if (tabLive) {
    tabLive.onclick = () => {
      console.log('Live Feed tab clicked');
      showPane(tabLive, livePane);
    };
  }
  if (tabUrls) {
    tabUrls.onclick = () => {
      console.log('API URLs tab clicked');
      showPane(tabUrls, urlsPane);
    };
  }
  if (tabStatus) {
    tabStatus.onclick = () => {
      console.log('System Status tab clicked');
      showPane(tabStatus, statusPane);
    };
  }
